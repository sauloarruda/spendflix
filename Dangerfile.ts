...
const typesChanged = danger.git.modified_files.some(f => f.includes('services/auth/src/types/api.d.ts'));

if (openapiChanged && !typesChanged) {
    warn('📝 You modified `openapi.yaml` but did not update the types. Please run `pnpm generate:types`.');
}
