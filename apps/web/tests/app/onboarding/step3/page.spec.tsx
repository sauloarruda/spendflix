...
jest.mock('next/image', () => ({
  __esModule: true,
  default: ({ alt, src, className, style, fill, ...restProps }: any) => {
    const imageProps: React.IMG_HTML_ATTRIBUTES<HTMLImageElement> = {
        alt: alt || '',
        src: src,
        className: className,
        style: style,
...
jest.mock('primereact/multiselect', () => {
  ...
  return {
    MultiSelect: (props: any) => {
      capturedMultiSelectOnChange = props.onChange;
      const {
        value,
        options, 
        placeholder, optionLabel, filter, display, itemTemplate, optionValue,
        className, style, id, 
        ...rest 
      } = props;
      return (
        <div data-testid="multiselect-other-banks-container" className={className} style={style} id={id} {...rest}>
...
jest.mock('@/components/onboarding/ResumeOnboarding', () => ({
  __esModule: true,
  default: ({ children, onError, message }: { children: React.ReactNode; onError?: () => void; message: string }) => (
    <div data-testid="resume-onboarding">
      {message && <div>{message}</div>}
      {children}
      {onError && <button data-testid="resume-onboarding-error" onClick={onError}>Trigger Error</button>}
    </div>
  ),
}));
...
describe('OnboardingStep3Page', () => {
  ...
  it('toggles selection of main banks and enables/disables continue button', async () => {
    ...
    act(() => { fireEvent.click(nubankCard); });
  ...
  it('shows multiselect on "Outro" click, and selection enables continue button', async () => {
    ...
    act(() => { fireEvent.click(otherCard); });
    await waitFor(() => expect(screen.getByTestId('multiselect-other-banks-container')).toBeInTheDocument());
  ...
  describe('handleContinue scenarios', () => {
    const selectBanks = async (mainBankNames: string[], otherBankCodes: string[]) => {
      for (const name of mainBankNames) {
        act(() => { fireEvent.click(getBankCard(name)); });
      }
      if (otherBankCodes.length > 0) {
        act(() => { fireEvent.click(getBankCard('Outro')); }); 
        await waitFor(() => expect(screen.getByTestId('multiselect-other-banks-container')).toBeInTheDocument());
        act(() => {
          capturedMultiSelectOnChange({ value: otherBankCodes });
        });
        await waitFor(() => expect(screen.getByTestId('multiselect-value-display')).toHaveTextContent(otherBankCodes.join(',')));
      }
    ...
    it('Scenario: Nubank + Other selected -> shows dialog, accepts -> navigates to step4', async () => {
      mockConfirmDialog.mockImplementationOnce((props: any) => { props.accept(); });
      render(<OnboardingStep3Page />);
      ...
      expect(updateOnboardingAction).toHaveBeenNthCalledWith(1, TEST_ONBOARDING_UID, {
        banks: expect.arrayContaining(['260', '025']), step: 4,
      });
      expect(mockConfirmDialog).toHaveBeenCalledTimes(1);
      expect(mockConfirmDialog.mock.calls[0][0].message).toContain('Atualmente só suportamos Nubank.');
    ...
    it('Scenario: Nubank + Other selected -> shows dialog, rejects -> navigates to waitlist', async () => {
      mockConfirmDialog.mockImplementationOnce((props: any) => { props.reject(); });
      render(<OnboardingStep3Page />);
      await selectBanks(['Nubank'], ['025']);
      const continueButton = screen.getByTestId('onboarding-navigation');
      await waitFor(() => expect(continueButton).not.toBeDisabled());
    ...
    it('Scenario: Only Other (non-Nubank) selected -> shows dialog, accepts -> navigates to waitlist', async () => {
      ...
      expect(mockConfirmDialog).toHaveBeenCalledTimes(1);
      expect(mockConfirmDialog.mock.calls[0][0].message).toContain('Infelizmente ainda não suportamos os bancos selecionados.');
      expect(updateOnboardingAction).toHaveBeenCalledTimes(1); 
      expect(mockRouterPush).toHaveBeenCalledWith('/onboarding/waitlist');
    });
